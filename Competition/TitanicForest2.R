...
# using MICE
  ...
  full$ageType[full$Age < 10] <- 'Young'
  full$ageType[full$Age >= 10] <- 'Adult'
  full$ageType[full$Age >= 60] <- 'Old'
  ...
  # changed to ageType
  rf_model <- randomForest(factor(Survived) ~ Pclass + Sex + AgeType +
                                              IndividualFare + Embarked + Title + 
                                              FsizeD + Child + Mother + HasCabin,
                                              data = train.titanic)
