...
# on 15/16 i added, then removed embarked and ismarried
...
# using MICE
  ...
  factor_vars <- c('PassengerId','Pclass','Sex','Embarked',
                   'Title','Surname','Family','FsizeD','Cabin','NameChar','MaidenName','IsMarried')
...
predictions <- predict(rf_model, test.titanic,type="response")

## calculate the confusion matrix
confusion <- table(predictions, test.titanic$Survived)
print(confusion)
...
submission.titanic <- data.frame(PassengerId = test.titanic$PassengerId, Survived = predictions)
